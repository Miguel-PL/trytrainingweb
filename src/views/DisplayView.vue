...
<template>
  <div class="bg-black text-white min-h-screen flex flex-col p-4">
    ...
    <div class="flex-1 flex flex-col gap-4">
      ...
      <div v-for="block in workout?.blocks" :key="block.name" class="flex-1 bg-gray-900 rounded p-4 flex flex-col">
        <h2 class="text-xl text-green-400 mb-2">
          {{ block.name }}
        </h2>

        <!-- Ejercicios -->
        <div class="flex-1 grid gap-2" :class="getGridClass(block.exercises.length || 0)">
          <div v-for="ex in block.exercises" :key="ex.id || ex.name"
            ...
            <div class="flex flex-col h-full">
              ...
              <video src="https://www.w3schools.com/html/mov_bbb.mp4" autoplay loop muted
                class="w-full h-2/3 object-cover rounded mb-1"></video>
